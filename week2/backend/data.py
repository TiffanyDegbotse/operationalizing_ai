...
<<<<<<< HEAD
        ...
        print("[NYC Cab Analytics] Model loaded.")
        return model
    except Exception as e:
        print(f"[NYC Cab Analytics] Error loading model: {e}")
=======
...
<<<<<<< HEAD
    baseline = _profile[(_profile["hour"] == hour) & (_profile["dayofweek"] == dow)].copy()
...
<<<<<<< HEAD
        ...
        synthetic_demand = base_demand * noise_factor * rush_hour_boost * time_pattern * zone_variance
...
def _calculate_lags_and_rolling(
...
def forecast_demand(zone_id: int, hour: int, dow: int, num_steps: int = 16, date: str = None) -> list:
    """
...
<<<<<<< HEAD
    ...
    demand = {str(int(r["PULocationID"])): round(float(r["avg"]), 2)
...
>>>>>>> upstream/main
    ...
    # Borough breakdown
<<<<<<< HEAD
    sub2 = sub.merge(_zones_df[["zone_id", "borough"]],
...
<<<<<<< HEAD
    ...
    return [{"hour": int(r["hour"]), "trips": round(float(r["avg"]), 1)}
            for _, r in sub.iterrows()]


def get_recommendations(zone_id: int, hour: int, dow: int, n: int = 3, holiday: str = "regular", date: str = None) -> list:
=======
            (_profile["PULocationID"] == zone_id)
            & (_profile["dayofweek"] == -1)
            & (_profile["holiday_name"] == holiday)
