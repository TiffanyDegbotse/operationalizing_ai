...
class Agent:
    ...
    def query(self, user_query: str, user_id: str = "default_user", user_role: str = "engineer") -> Dict[str, Any]:
        logger.info(f"Processing query: {user_query}")
        if not self.rate_limiter.is_allowed(user_id):
            return {"answer": "Rate limit exceeded. Please wait before making another query.", "tokens_used": 0, "cost": 0.0, "role": user_role}
        if not self.cost_enforcer.can_afford_query(user_id, estimated_cost=0.01, role=user_role):
            return {"answer": "Budget exceeded for your role.", "tokens_used": 0, "cost": 0.0, "role": user_role}
        total_tokens = 0
